...
1. <u>Device tab</u>
    ...
    * "Updates Firmware" 실행시, Firmware 변경.
      + "information" group box 에서 "full info" 선택시
        - [x] advance mode 의 file 선택 dialog 표시.
        - rom file 선택시.
          + [x] 정보창에 rom 파일 정보 표시.
          + [x] combobox 에 rom 내의 firmware 표시.
          + [x] 현재 장비와 다른 이름의 firmware 최종 선택시 경고 표시.
          + [x] 최종 선택된 firmware 로 업데이트 성공.
        - raw binary file 선택시.
          + [x] 정보창에 선택 파일 정보 표시.
          + [x] combobox 요소 삭제.
          + [x] 최종 선택된 firmware 로 업데이트 성공.
      + "information" group box 에서 "full info" 이 외 선택시
        - [x] normal mode 의 file 선택 dialog 표시.
        - rom file 선택시, Device name이 같고, firmare 의 버전이 현재 system version 이상 일때 만 업데이트 시작.
          ...
          + [x] ganymede v5.21 firmware 가 있는 장비에 lpu23x_00027.rom 업데이트 성공.
          + [x] europa v1.0 firmware 가 있는 장비에 lpu23x_00027.rom 업데이트 성공.
          + [ ] callisto v3.23 firmware 가 있는 장비에 lpu23x_00027.rom 업데이트 성공.
          + [x] ganymede v5.22 firmware 가 있는 장비에 lpu23x_00027.rom 업데이트 성공.
          + [x] europa v1.1 firmware 가 있는 장비에 lpu23x_00027.rom 업데이트 성공.
        - raw binary file 선택시.
          + [x] europa firmware의 interface 라 usb vcom 인 경우, ganymede binary 로 강제 update를 하면, update 후, interface 가 usb keyboard 로 자동 변경.
          + [x] ganymede firmware의 interface 라 usb keyboard 인 경우, europa binary 로 강제 update를 하면, update 후, interface 가 usb vcom 로 자동 변경.

2. <u>Common tab</u>
    * [ ] "Selects Interface" drop list 변경 후, "Apply" 실행시, device interface 변경.
...
10. <u>setting file</u>
    * [x] lpu230.xml 파일이 lpu230.exe 와 같은 폴더, %ProgramData%\Elpusk\00000006\lpu230 폴더, 그리고 %ProgramData%\Easyset\00000000\lpu230 폴더에 3개 존재 할때, lpu230.exe 와 같은 폴더에 있는 lpu230.xml 를 사용한다.
    * [x] lpu230.xml 파일이 %ProgramData%\Elpusk\00000006\lpu230 폴더 그리고 %ProgramData%\Easyset\00000000\lpu230 폴더에 2개 존재 할때, %ProgramData%\Elpusk\00000006\lpu230 폴더에 있는 lpu230.xml 를 사용한다.
    * [x] lpu230.xml 파일이 %ProgramData%\Easyset\00000000\lpu230 폴더에 1개 존재 할때, %ProgramData%\Easyset\00000000\lpu230 폴더에 있는 lpu230.xml 를 사용한다.
    * [x] 위 3가지 폴더에 lpu230.xml 이 없으면, default 설정(log file 미생성,update 복구를 위한 임시화일 생성 위치를 lpu230.exe 가 있는 폴더)을 따른다.
    * lpu230.xml "log" element 의 enable attribute 값이 "true" 일 때만 log file를 생성.
    * [x] lpu230.exe 와 같은 폴더에 있는 lpu230.xml 이 사용될 때, firmware update 복구 파일은 lpu230.exe 와 같은 폴더에 생성.
    * [x] %ProgramData%\Elpusk\00000006\lpu230 폴더에 있는 lpu230.xml 가 사용 될 때, firmware update 복구 파일은 %ProgramData%\Elpusk\00000006\lpu230\data 폴더에 생성.
    * [x] %ProgramData%\Easyset\00000000\lpu230 폴더에 있는 lpu230.xml 가 사용 될 때, firmware update 복구 파일은 %ProgramData%\Easyset\00000000\lpu230\data 폴더에 생성.